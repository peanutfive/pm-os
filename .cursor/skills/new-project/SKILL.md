...
- **不能**在任何现有 git 仓库内（跑一下 `git -C <父目录> rev-parse --show-toplevel` 检查；有返回 = 有仓库 = 换位置）
...
if [ -e "$TARGET" ]; then echo "⚠️ 目录已存在，停下来找用户确认"; fi
if git -C "$(dirname "$TARGET")" rev-parse --show-toplevel 2>/dev/null; then
  echo "⚠️ 父目录在一个 git 仓库内，需换位置"
fi
```
...
| 反模式 | 修法 |
|---|---|
| 在现有仓库的子目录里 `git init` 新项目 | 把文件移到 `~/developer/<new-name>/` 再 `git init` |
